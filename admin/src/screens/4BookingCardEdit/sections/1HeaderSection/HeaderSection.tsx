...
export const HeaderSection = ({
    ...
    useEffect(() => {
        if (!categories.length) return; // ждём, пока категории загрузятся

        if (category) {
            const selected = categories.find((c) => c.id === category);
            setAvailableSubcategories(selected ? selected.subCategories : []);

            // Если выбранная подкатегория больше не существует — сброс
            if (subcategory && selected) {
                const exists = selected.subCategories.some((s) => s.id === subcategory);
                if (!exists) setSubcategory(null);
            }
        } else {
            setAvailableSubcategories([]);
            setSubcategory(null);
